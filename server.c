...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // look for space in line
            char* ptr = strchr(line, ' ');
            
            // if pointer does not exist, return error
            if (ptr == NULL)
            {
                error(400);
                continue;
            ...
            // else replace space with \0
            *ptr = '\0';
            
            // set pointer to point at start of request-target
            ptr++;
            
            // if method is not GET, yell at user 
            ...
            // create pointer for HTTP
            char* httpptr = strchr(ptr, ' ');
            ...
            *httpptr = '\0';
            ...
            // set pointer to point at start of HTTP-version
            httpptr++;
            
            // if CRLF does not exist, yell at user
            if (strstr(httpptr, "\r\n") == NULL)
            {
                error(400);
                continue;
            ...
            // if HTTP-version is not what we want, yell at user
            if (strstr(httpptr, "HTTP/1.1") == NULL)
            {
                error(505);
                continue;
            }
            
            // if request-target does not begin with /, yell at user
            if (*ptr != '/')
            {
                error(501);
                continue;
            ...
            // if request-target contains ", yell at user
            if (strchr(ptr, '\"') != NULL)
            {
                error(400);
                continue;
            ...
            // create pointer for query
            char* query = strchr(ptr, '?');
            
            // if pointer exists, replace ? with \0
            if (query != NULL)
            {
                *query = '\0';
                
                // set pointer to point at start of query
                query++;
            }

			// else if pointer doesn't exist, set it to non-NULL
			else
			{
			query = httpptr;
			}
            
            // create pointer for extension
            char* extension = strchr(ptr, '.');
            if (extension == NULL)
            {
                error(501);
                continue;
            ...
            // set pointer to point at start of extension
            extension++;

            // TODO: extract query from request-target
            // included in previous TODO -- char* query points to query

            // TODO: concatenate root and absolute-path
            char path[strlen(root) + strlen(ptr) + 1];
            
            // copy root into path
            strcpy(path, root);
            
            // concatenate absolute path with root
            strcat(path, ptr);
			...
			// TODO: validate request-line~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~end~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
